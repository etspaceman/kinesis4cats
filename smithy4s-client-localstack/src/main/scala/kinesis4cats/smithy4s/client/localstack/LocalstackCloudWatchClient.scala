...
/** Like CloudWatchClient, but also includes the
...
object LocalstackCloudWatchClient {
  ...
  /** Creates a [[cats.effect.Resource Resource]] of a CloudWatchClient that is
  ...
  def clientResource[F[_]](
      ...
      LE: LogEncoders[F],
      LELC: LogEncoder[LocalstackConfig]
  ): Resource[F, CloudWatchClient[F]] = LocalstackConfig
    .resource(prefix)
